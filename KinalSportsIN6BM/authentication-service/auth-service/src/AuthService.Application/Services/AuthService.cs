    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    // Si no hay credenciales SMTP reales, el correo de verificacion nunca
    // llega (ver EmailService.SendEmailAsync, que revienta silenciosamente
    // en el Task.Run de background). Sin este fallback, cualquier usuario
    // nuevo quedaria con Status=false para siempre y no podria iniciar
    // sesion nunca (401 "User account is disabled"). En desarrollo local,
    // activamos la cuenta de inmediato; en un entorno con SMTP configurado
    // de verdad, se respeta el flujo normal de verificacion por email.
    private bool IsSmtpConfigured()
    {
        var smtp = configuration.GetSection("SmtpSettings");
        return !string.IsNullOrWhiteSpace(smtp["Username"])
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        var user = new User
        {
            ...
            UserEmail = new UserEmail
            {
                ...
                EmailVerificationToken = smtpConfigured ? emailVerificationToken : null,
                EmailVerificationTokenExpiry = smtpConfigured ? DateTime.UtcNow.AddHours(24) : null
            },
            UserRoles =
            [
        ...
        // Enviar email de verificación en background (solo si SMTP esta configurado)
        if (smtpConfigured)
        {
            _ = Task.Run(async () =>
            {
        ...
        {
            logger.LogWarning("SMTP no configurado: la cuenta se activo automaticamente sin verificacion de email.");
        }
        ...
        return new RegisterResponseDto
        {
            ...
            Message = smtpConfigured
                ...
                : "Usuario registrado exitosamente. Tu cuenta ya esta activa.",
