...
// Helper puro: cuenta cuántos platos usan una categoría (por _id o por nombre)
export const countDishesInCategory = (categoryId, categoryName, dishes = []) => {
    return dishes.filter((dish) => {
        const cat = dish.category;
        if (!cat) return false;
        if (cat._id) return cat._id === categoryId;
        if (typeof cat === "string") return cat === categoryId || cat === categoryName;
        return false;
    }).length;
};
...
export const Categories = () => {
    ...
    useEffect(() => {
        getCategories();
        getDishes(); // para calcular cuántos platos usa cada categoría
    }, [getCategories, getDishes]);
        ...
        <div className="p-6">
            ...
            <div style={{ background: "#141210", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "12px", overflow: "hidden" }}>
                <div className="grid grid-cols-12 px-6 py-3 text-xs uppercase tracking-widest"
                    ...
                    <span className="col-span-1">Platos</span>
                ...
                {categories.length === 0 ? (
                    ...
                    categories.map((cat, index) => {
                        const dishCount = countDishesInCategory(cat._id, cat.name, dishes);
                        return (
                            <div key={cat._id}
                            ...
                            >
                                ...
                                <span className="col-span-4 text-sm truncate pr-4" style={{ color: "#9a8e74" }}>
                                    {cat.description || "—"}
                                </span>

                                {/* Columna nueva: cuántos platos usan esta categoría */}
                                <span className="col-span-1 text-sm" style={{ color: dishCount > 0 ? "#c9a84c" : "#3a3328" }}>
                                    {dishCount > 0 ? (
                                        <span title={`${dishCount} plato${dishCount !== 1 ? "s" : ""} asignado${dishCount !== 1 ? "s" : ""}`}>
                                            {dishCount}
                                        </span>
                                    ) : "—"}
                                </span>
                                ...
                                <div className="col-span-2 flex justify-end gap-2">
                                    ...
                                    <button
                                        onClick={() => openConfirm({
                                            ...
                                            message: dishCount > 0
                                                ? `"${cat.name}" tiene ${dishCount} plato${dishCount !== 1 ? "s" : ""} asignado${dishCount !== 1 ? "s" : ""}. ¿Eliminar de todas formas?`
                                                : `¿Eliminar "${cat.name}"?`,
