...
export const Reports = () => {
    const { stats, loading, error, getGeneralStats } = useReportStore();
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-8">
                <div>
                    ...
                    <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2rem", fontWeight: 300, color: "#f0e8d5", lineHeight: 1.2 }}>
                        Reportes <em style={{ color: "#c9a84c", fontStyle: "italic" }}>Generales</em>
                    </h1>
                </div>
                <button
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {stats ? (
                    [
                        ...
                        { label: "Calificación promedio", value: stats.calificacionPromedio },
                    ].map((item) => (
                        <div key={item.label} className="rounded-3xl p-6" style={{ background: "#141210", border: "1px solid rgba(201,168,76,0.15)" }}>
                            ...
                            <p className="text-3xl font-semibold" style={{ color: "#f0e8d5" }}>{item.value ?? "—"}</p>
                        </div>
                    ))
                ) : (
