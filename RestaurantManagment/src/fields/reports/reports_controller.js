...
export const getGeneralStats = async (req, res) => {
    try {
        const [totalRestaurants, totalOrders, totalReservations, totalEvents, totalDishes, revenueAgg, avgRatingAgg, reservationsByStatus] = await Promise.all([
            Restaurant.countDocuments({ isActive: true }),
            Order.countDocuments(),
            Reservation.countDocuments(),
            Event.countDocuments(),
            Dish.countDocuments({ isAvailable: true }),
            Order.aggregate([{ $match: { status: 'paid' } }, { $group: { _id: null, totalRevenue: { $sum: '$total' }, avgTicket: { $avg: '$total' } } }]),
        ...
        return res.status(200).json({
            success: true,
            stats: {
                restaurantes: totalRestaurants, ordenes: totalOrders, reservaciones: totalReservations,
                eventos: totalEvents, platillos: totalDishes,
                ingresosTotales: revenueAgg[0]?.totalRevenue ?? 0,
                ticketPromedio: revenueAgg[0]?.avgTicket ?? 0,
                calificacionPromedio: avgRatingAgg[0]?.avgRating?.toFixed(2) ?? 0,
                reservacionesPorEstado: reservationsByStatus
            }
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al obtener estadísticas generales', error: error.message });
    }
};
...
export const getTopDishes = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, total: topDishes.length, topDishes });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al obtener platos más vendidos', error: error.message });
    }
};
...
export const getPeakHours = async (req, res) => {
    try {
        ...
        const peakHours = await Order.aggregate([
            { $match: matchStage },
            { $group: { _id: { $hour: '$createdAt' }, totalOrdenes: { $sum: 1 }, ingresoTotal: { $sum: '$total' } } },
            { $sort: { totalOrdenes: -1 } },
            { $project: { hora: '$_id', totalOrdenes: 1, ingresoTotal: 1, _id: 0 } }
        ]);
        return res.status(200).json({ success: true, horasPico: peakHours, horaMasConcurrida: peakHours[0]?.hora ?? null });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al obtener horas pico', error: error.message });
    }
};
...
export const getRestaurantStats = async (req, res) => {
    try {
        ...
        const [ordersStats, reservationsStats, reviewsStats, eventsStats, ordersPerDay, ordersByType] = await Promise.all([
            Order.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: null, totalOrdenes: { $sum: 1 }, ingresosTotal: { $sum: '$total' }, ticketPromedio: { $avg: '$total' } } }]),
            Reservation.aggregate([{ $lookup: { from: 'tables', localField: 'table', foreignField: '_id', as: 'tableInfo' } }, { $unwind: '$tableInfo' }, { $match: { 'tableInfo.restaurant': rId } }, { $group: { _id: null, totalReservaciones: { $sum: 1 }, personasPromedio: { $avg: '$numberOfPeople' } } }]),
            Review.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: null, totalReseñas: { $sum: 1 }, calificacionPromedio: { $avg: '$rating' }, dist5: { $sum: { $cond: [{ $eq: ['$rating', 5] }, 1, 0] } }, dist4: { $sum: { $cond: [{ $eq: ['$rating', 4] }, 1, 0] } }, dist3: { $sum: { $cond: [{ $eq: ['$rating', 3] }, 1, 0] } }, dist2: { $sum: { $cond: [{ $eq: ['$rating', 2] }, 1, 0] } }, dist1: { $sum: { $cond: [{ $eq: ['$rating', 1] }, 1, 0] } } } }]),
            Event.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
            ...
            Order.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: '$type', count: { $sum: 1 } } }])
        ]);

        return res.status(200).json({
            success: true,
            restaurante: { id: restaurant._id, nombre: restaurant.name, calificacion: restaurant.averageRating, totalReseñas: restaurant.totalReviews },
            estadisticas: {
                ordenes: { total: ordersStats[0]?.totalOrdenes ?? 0, ingresos: ordersStats[0]?.ingresosTotal ?? 0, ticketPromedio: ordersStats[0]?.ticketPromedio?.toFixed(2) ?? 0 },
                reservaciones: { total: reservationsStats[0]?.totalReservaciones ?? 0, personasPromedio: reservationsStats[0]?.personasPromedio?.toFixed(1) ?? 0 },
                reseñas: { total: reviewsStats[0]?.totalReseñas ?? 0, calificacionPromedio: reviewsStats[0]?.calificacionPromedio?.toFixed(2) ?? 0, distribucion: { '5_estrellas': reviewsStats[0]?.dist5 ?? 0, '4_estrellas': reviewsStats[0]?.dist4 ?? 0, '3_estrellas': reviewsStats[0]?.dist3 ?? 0, '2_estrellas': reviewsStats[0]?.dist2 ?? 0, '1_estrella': reviewsStats[0]?.dist1 ?? 0 } },
                eventos: eventsStats, ordenesPorDia: ordersPerDay, ordenesPorTipo: ordersByType
            }
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al obtener estadísticas del restaurante', error: error.message });
    }
};
...
export const getDemandReport = async (req, res) => {
    try {
        ...
        const [ordersPerDay, reservationsPerDay, revenuePerDay] = await Promise.all([
            Order.aggregate([{ $match: matchOrder }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } }, ordenes: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            Reservation.aggregate([{ $match: { ...buildDateFilter(from, to) } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, reservaciones: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            Order.aggregate([{ $match: { ...matchOrder, status: 'paid' } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } }, ingresos: { $sum: '$total' } } }, { $sort: { _id: 1 } }])
        ]);
        return res.status(200).json({
            success: true,
            filtros: { desde: from || 'todos', hasta: to || 'todos', restauranteId: restaurantId || 'todos' },
            demanda: { ordenesPorDia: ordersPerDay, reservacionesPorDia: reservationsPerDay, ingresosPorDia: revenuePerDay }
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al obtener reporte de demanda', error: error.message });
    }
};
...
export const sendGeneralStatsPDF = async (req, res) => {
    try {
        ...
        const data = [
            ...
            { concepto: 'Calificación Promedio del Sistema', valor: reviewsAgg[0]?.promedio?.toFixed(2) ?? 'N/A', detalle: `Total de reseñas: ${reviewsAgg[0]?.total ?? 0}` }
        ];

        const STATS_FIELDS = [{ label: 'Concepto', key: 'concepto' }, { label: 'Valor', key: 'valor' }, { label: 'Detalle', key: 'detalle' }];
        const service = new EmailPDFService();
        const result  = await service.sendEntityPDF({ toEmail: email, subject: 'Reporte General del Sistema – Gestión Restaurante', title: 'Estadísticas Generales del Sistema', entityName: 'Estadísticas Generales', data, fields: STATS_FIELDS, filename: 'estadisticas_generales.pdf' });
        return res.status(200).json({ success: true, message: `PDF enviado correctamente a ${result.toEmail}`, data: { correoDestino: result.toEmail, archivoEnviado: result.filename, totalConceptos: result.records } });
    } catch (error) {
...
export const sendRestaurantStatsPDF = async (req, res) => {
    try {
        ...
        const [ordersStats, reservationsStats, reviewsStats, topDishesRaw] = await Promise.all([
            Order.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: '$status', count: { $sum: 1 }, ingresos: { $sum: '$total' } } }]),
            Reservation.aggregate([{ $lookup: { from: 'tables', localField: 'table', foreignField: '_id', as: 'tableInfo' } }, { $unwind: '$tableInfo' }, { $match: { 'tableInfo.restaurant': rId } }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
            Review.aggregate([{ $match: { restaurant: rId } }, { $group: { _id: null, promedio: { $avg: '$rating' }, total: { $sum: 1 } } }]),
            OrderDetail.aggregate([{ $lookup: { from: 'dishes', localField: 'dish', foreignField: '_id', as: 'dishInfo' } }, { $unwind: '$dishInfo' }, { $match: { 'dishInfo.restaurant': rId } }, { $group: { _id: '$dish', nombre: { $first: '$dishInfo.name' }, totalVendido: { $sum: '$quantity' }, ingresos: { $sum: '$subtotal' } } }, { $sort: { totalVendido: -1 } }, { $limit: 5 }])
        ]);

        const data = [
            { concepto: 'Restaurante', valor: restaurant.name, detalle: restaurant.address },
            { concepto: 'Calificación Promedio', valor: restaurant.averageRating, detalle: `${restaurant.totalReviews} reseñas` },
            ...ordersStats.map(o => ({ concepto: `Órdenes (${o._id})`, valor: o.count, detalle: `Q${o.ingresos?.toFixed(2)}` })),
            ...reservationsStats.map(r => ({ concepto: `Reservaciones (${r._id})`, valor: r.count, detalle: '' })),
            { concepto: 'Satisfacción del cliente', valor: reviewsStats[0]?.promedio?.toFixed(2) ?? 'N/A', detalle: `Total: ${reviewsStats[0]?.total ?? 0}` },
            ...topDishesRaw.map((d, i) => ({ concepto: `Top ${i + 1} Plato: ${d.nombre}`, valor: `${d.totalVendido} vendidos`, detalle: `Q${d.ingresos?.toFixed(2)}` }))
        ];

        const STATS_FIELDS = [{ label: 'Concepto', key: 'concepto' }, { label: 'Valor', key: 'valor' }, { label: 'Detalle', key: 'detalle' }];
        const service = new EmailPDFService();
        const result  = await service.sendEntityPDF({ toEmail: email, subject: `Estadísticas del Restaurante – ${restaurant.name}`, title: `Reporte de Desempeño: ${restaurant.name}`, entityName: 'Restaurante', data, fields: STATS_FIELDS, filename: `stats_${restaurant.name.replace(/\s+/g, '_')}.pdf` });
        return res.status(200).json({ success: true, message: `PDF enviado correctamente a ${result.toEmail}`, data: { correoDestino: result.toEmail, archivoEnviado: result.filename, restaurante: restaurant.name } });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al enviar el PDF', error: error.message });
    }
};
...
export const sendTopDishesPDF = async (req, res) => {
    try {
        const { email } = req.params;
        const limit = parseInt(req.query.limit) || 10;
        const { restaurantId } = req.query;
        ...
        const DISH_FIELDS = [{ label: 'Platillo', key: 'nombre' }, { label: 'Precio Unitario', key: 'precio' }, { label: 'Unidades Vendidas', key: 'totalVendido' }, { label: 'Ingreso Generado', key: 'ingresoGenerado' }];
        const service = new EmailPDFService();
        const result  = await service.sendEntityPDF({ toEmail: email, subject: `Top ${limit} Platillos Más Vendidos`, title: `Top ${limit} Platillos Más Vendidos`, entityName: 'Platillo', data: topDishes, fields: DISH_FIELDS, filename: 'top_platillos.pdf' });
        return res.status(200).json({ success: true, message: `PDF enviado correctamente a ${result.toEmail}`, data: { correoDestino: result.toEmail, archivoEnviado: result.filename, platillosIncluidos: result.records } });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error al enviar el PDF', error: error.message });
    }
};
